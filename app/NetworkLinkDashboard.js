...
export default class NetworkLinkDashboard extends React.Component {
  ...
  render() {
    ...
    if (this.state.topologyJson && this.state.topologyJson.sites) {
      ...
      // for each link, check mac <-> mac, graph snr, rssi, tx/rx
      ...
      // same for link metrics
//      console.log(chartData, graphOptions);
/*      gridComponents = links.map(link => {
        return (
          <li key={link.a_node.name + "-" + link.z_node.name + "-li"}>
            <ReactMultiGraph
              a_node={link.a_node}
              z_node={link.z_node}
              metrics={["bandwidth", "snr", "rssi"]}
              size="large"
            />
          </li>
        );
    ...
    if (this.state.linksSelected.length &&
      ...
      this.state.linkMetricsSelected.forEach(metric => {
        graphOptions.push({
          type: 'link',
          a_node: 'a',
          z_node: 'z',
        ...
        <ScaleModal ref="selector">
          ...
          <div width="700">
            <BootstrapTable
              ...
              <TableHeaderColumn dataField="mac_addr" width="200">
                MAC
              </TableHeaderColumn>
            </BootstrapTable>
            <BootstrapTable
