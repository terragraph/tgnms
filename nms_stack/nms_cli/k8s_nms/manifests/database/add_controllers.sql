...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM topology WHERE name = ctrlr_name) then
    INSERT INTO controller (api_ip, e2e_ip, e2e_port, api_port) VALUES (CONCAT(sanitized_name), CONCAT(sanitized_name), app_port, 8080);
...
{% set sanitized_name = ctrlr.name | trim | replace(' ', '_') | replace('_', '-') | lower %}
