...
class ConnectivityTests(unittest.TestCase):
    def setUp(self) -> None:
        config = {
            ...
            "beam_order_range": [[31, -1, -1], [32, 64, 1]],
        }

        # Configure connectivity analysis constants
        beam_order: List[int] = []
        for start, end, interval in config["beam_order_range"]:
            beam_order += list(range(start, end, interval))
        self.con = HardwareConfig(beam_order, **config["constants"])

    def test_seperate_beams(self) -> None:
        routes = [(0, 0, 11), (5, 0, 10), (20, 20, 5)]
        separate_beams(routes, self.con)
    ...
    def test_find_routes_compute_empty(self) -> None:
        ...
        routes = find_routes_compute(beam_map, 20, 10, self.con)
    ...
    def test_find_routes_snr(self) -> None:
        ...
        routes = find_routes(im_data, 10, False, self.con)
    ...
    def test_find_routes_rssi(self) -> None:
        ...
        routes = find_routes(im_data, -70, True, self.con)
