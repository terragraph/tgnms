...
export default class UpgradeLeftPaneStatus extends React.Component {
  ...
  render() {
    ...
    const currentRequestId = (currentRequest) ? (
      ...
      <span style={{color: '#777'}}>There are no upgrade requests currently running</span>
    );

    const pendingRequestIds = pendingRequests.length > 0 ? pendingRequests.map((req) => {
      return (
        <p>{req.urReq.upgradeReqId}</p>
      );
    }) : (
      <span style={{color: '#777'}}>There are no pending upgrade requests</span>
    );
