...
export default class DetailsTopologyIssues extends React.Component {
  ...
  addSitesAsync(remainingSites) {
    ...
    request.onload = function() {
      ...
      if (request.status == 200) {
        ...
        swal({
          title: "Failed!",
          text: "Adding site '" + siteToAdd.name + "' failed\nReason: "+request.statusText,
          type: "error"
        });
        this.setState({processing: false});
      }
    ...
    if (isNew) {
      ...
      Object.keys(site.nodes).forEach(nodeName => {
        ...
        node.links.forEach(link => {
          ...
          // viewable
          let remoteNodeName = link.a_node_name == node.node.name ?
  ...
  remoteSiteInfo(localSite, linkObj, localNodeName) {
    ...
    let remoteNodeName = linkObj.a_node_name == localNodeName ?
                         linkObj.z_node_name : linkObj.a_node_name;
    // find remote site (z)
    let ret = {};
    let remoteSite;
    if (this.nodesByName.hasOwnProperty(remoteNodeName)) {
  ...
  render() {
    ...
    this.props.newTopology.nodes.forEach(node => {
      ...
      this.newSitesByName[node.site_name].nodes[node.name] = {node: node, links: []};
    });
    ...
    Object.keys(this.newSitesByName).forEach((siteName, i) => {
      ...
      if (this.sitesByName.hasOwnProperty(siteName)) {
        ...
        Object.values(newSiteByName.nodes).forEach((nodeLinkObj, i) => {
          ...
          nodesAndLinks.push(
            <tr>
              <td>Node</td>
              <td>
                <span onClick={() => {this.selectNode(node.name)}}>
                  {node.name}
                </span>
              </td>
          ...
          links.forEach((link, i) => {
            ...
            let remoteSiteInfo = this.remoteSiteInfo(newSiteByName.site,
                                                     link, node.name);
            nodesAndLinks.push(
              <tr>
        ...
        issues.push(
          <tr>
            <td colSpan="5" className={this.classNameIsNew(true)}
                            style={{borderTop: '2px solid cadetblue'}}>
              <span onClick={() => {this.selectSite(newSiteByName, true)}}
        ...
        <div className="details-content">
          ...
          <div className="details-body"
            ...
            <table className="details-table"
                   style={{width: '100%', borderLeft: '2px solid cadetblue',
