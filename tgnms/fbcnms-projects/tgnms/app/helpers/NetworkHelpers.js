...
export function isNodeAlive(nodeStatus: number) {
  return (
...
export function approxDistance(l1: LocationType, l2: LocationType) {
...
export function availabilityColor(livenessPercent: number) {
...
export function renderAvailabilityWithColor(
  livenessPercentString: string | number | void,
) {
...
export function renderSnrWithColor(snr: number) {
  const color =
    snr >= SNR_THRESHOLD_MCS2
      ? snr >= SNR_THRESHOLD_MCS9
        ...
        : 'yellowgreen'
      : 'red';
  return <span style={{color}}>{formatNumber(snr, 2)}dB</span>;
}

/** Returns the SNR as an icon. */
export function renderSnrWithIcon(snr: number, props: {} = {}) {
 ...
 */
export function mapboxShouldAcceptClick(evt: any) {
 ...
 **/
export function hasNodeEverGoneOnline(
  node: NodeType,
...
export function hasLinkEverGoneOnline(
  link: LinkType | TemporaryLinkType,
  offline_whitelist: ?OfflineWhiteListType,
): boolean {
  // if no offline whitelist, assume it has gone online in the past
