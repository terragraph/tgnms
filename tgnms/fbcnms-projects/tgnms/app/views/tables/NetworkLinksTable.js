...
class NetworkLinksTable extends React.Component<Props, State> {
    ...
    Object.keys(context.linkMap).forEach(linkName => {
      ...
      rows.push({
        ...
        tx_beam_angle: formatNumber(
          beamIndexToAngle(analyzerLinkA.tx_beam_idx),
        ),
        rx_beam_angle: formatNumber(
          beamIndexToAngle(analyzerLinkA.rx_beam_idx),
        ),
        distance: link._meta_.distance,
      });
      ...
      rows.push({
        ...
        tx_beam_angle: formatNumber(
          beamIndexToAngle(analyzerLinkZ.tx_beam_idx),
        ),
        rx_beam_angle: formatNumber(
          beamIndexToAngle(analyzerLinkZ.rx_beam_idx),
        ),
        distance: link._meta_.distance,
      });
    });
  ...
  renderAlivePerc(cell: number, row) {
    let cellColor = 'red';
    let cellText = '-';
    if (row.type === 'Wired') {
