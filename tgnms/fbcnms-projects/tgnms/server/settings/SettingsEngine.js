...
const DISABLE_ENV_DIR_KEY = 'DISABLE_ENV_DIR';
...
export default class SettingsEngine {
  ...
  initialize = (SETTINGS: Array<SettingDefinition>): SettingsState => {
    ...
    if (typeof process.env[DISABLE_ENV_DIR_KEY] === 'undefined') {
      try {
        const envDir = this._getSettingsDir();
        const files = fs.readdirSync(envDir);
        for (const file of files) {
          // Read all .env files except '.env' itself, which is handled above
          // by DISABLE_ENV_FILE_KEY
          if (file.endsWith('.env') && file !== '.env') {
            dotenv.config({path: path.join(envDir, file), debug: 'true'});
          }
        }
      } catch (e) {
        logger.error(`Error parsing env files in env dir: ${e.message}`);
      }
    } else {
      logger.warn('env directory disabled.');
    }
