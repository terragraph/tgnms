 ...
 public:
  ...
  class ReadCallback {
   public:
     ...
     * We pass the pointer to a properly filled struct msghdr*, which among
     * others defines the receive and control buffer. See recvmsg(2) for more
     * details.
     */
    virtual void getMessageHeader(struct msghdr** msg) noexcept = 0;

    /**
     * Invoked when a new datagraom is available on the socket. `len` is the
     * number of bytes read. The rest of the metadata is set in the struct
     * msghdr previously passed to the socket.
     */
  ...
  /**
   * Bind the socket to the following address. If port is not set in the
   * 'address' an ephemeral port is chosen and you can use the 'address()'
