...
STACK_TRACE_LINE_RE = re.compile("^#\d+\s+([0-9a-fA-FxX]*)\s+in\s+(.+)\s+\(\)(\s+)?(from)?(.+)?")
...
class CrashAnalyzer:
  ...
  @staticmethod
  def extract_time_from_log_path(log_path: str) -> Optional[str]:
  ...
  def parse_stack_trace_log(
      self, log_lines: List[str], node_id: str, application: str,
      crash_time: Optional[str], num_lines_before: int=25, num_lines_after: int=5
    ...
    if match is not None:
      ...
      affected_lines = log_lines[max(0, i-num_lines_before):i+num_lines_after]
      ...
      current_thread = self.extract_current_thread(maybe_thread_str=log_lines[i+1])
      if current_thread is not None:
        # Update the affected lines with the current thread's stack trace
        thread_stack_trace = self._extract_thread_stack_trace(thread=current_thread, lines=log_lines)
      ...
      crash_time = specific_crash_time if specific_crash_time is not None else crash_time
      return CrashDetails(crash_type=crash_type, crash_time=crash_time, node_id=node_id,
                          application=application, affected_function=function_name,
                          affected_lines=affected_lines)
    return None

  def parse_crash_stack_trace(
      self, crash_type: str, maybe_stack_trace: List[str], node_id: str, application: str
    ) -> Optional[CrashDetails]:
  ...
  def find_error_msg(
      self, error_msg_re_str: str, log_lines: List[str], node_id: str, application: str,
      crash_time: Optional[str], num_lines_before: int=25, num_lines_after: int=5
    ) -> List[CrashDetails]:
    ...
    for i in range(0, len(log_lines)):
      ...
      if match is not None:
        ...
        maybe_crash_details = self.parse_crash_stack_trace(
                                crash_type=match.group(1), maybe_stack_trace=log_lines[i+1:i+20],
                                node_id=node_id, application=application)
        if maybe_crash_details is not None:
          ...
          found_errors.append(
            CrashDetails(crash_type=error_msg, crash_time=crash_time, node_id=node_id,
                        application=application, affected_function=None,
                        affected_lines=log_lines[max(0, i-num_lines_before):i+num_lines_after]))
    return found_errors
  ...
  def run_error_parsers(
    ...
    if "stack" in log_path:
      ...
      stack_trace_details = self.parse_stack_trace_log(
                              log_lines=log_lines, node_id=node_id,
                              application=application, crash_time=crash_time)
      # If the parsing was successful, add to found_crashes
      if stack_trace_details is not None and stack_trace_details.crash_type is not None:
    ...
    if len(found_crashes) == 0:
      for error_msg_re_str in self.ERROR_MSGS_RE:
        found_crashes.extend(self.find_error_msg(
                              error_msg_re_str=error_msg_re_str, log_lines=log_lines,
                              node_id=node_id, application=application, crash_time=crash_time))
