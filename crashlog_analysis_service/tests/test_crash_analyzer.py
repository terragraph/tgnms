...
class CrashAnalyzerTestCase(unittest.TestCase):
  ...
  def test_extract_thread_stack_trace(self):
    ...
    for test_thread in test_cases:
      ...
      test_res = crashAnalyzer._extract_thread_stack_trace(test_thread, self.stack_trace_lines)
  ...
  def test_parse_stack_trace_log(self):
    ...
    for test_file in test_cases:
      ...
      self.assertEqual(len([] if test_res == None else test_res.affected_lines), exp_res)
  ...
  def test_find_error_msg(self):
    ...
    test_cases: Dict[(str, str), (int, int)] = {
      ...
      (FW_ERROR_LOG_FILE_PATH, "(Firmware error) detected, assert codes"): (1, 29),
    }
    crashAnalyzer = CrashAnalyzer()
  ...
  def test_run_error_parsers(self):
    ...
    for test_file in test_cases:
      exp_res = test_cases[test_file]
      test_res = crashAnalyzer.run_error_parsers(test_file, self.file_path_to_lines[test_file], "", "")
