...
class VppCrashAnalyzerTestCase(unittest.TestCase):
  ...
  def test_parse_crash_stack_trace(self):
    ...
    for test_file, error_msg in test_cases:
      ...
      test_res = vppCrashAnalyzer.find_error_msg(error_msg, self.file_path_to_lines[test_file], "", "", None)
      self.assertEqual(len(test_res) if test_res is not None else 0, exp_res_first)
  ...
  def test_run_error_parsers(self):
    ...
    for test_file in test_cases:
      ...
      test_res = vppCrashAnalyzer.run_error_parsers(test_file, self.file_path_to_lines[test_file], "", "")
